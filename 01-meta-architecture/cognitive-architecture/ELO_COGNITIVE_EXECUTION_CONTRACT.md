...
`REVALIDATING` is an execution alias for the validation phase and MUST resolve to `VALIDATING` in durable state unless an implementation requires a separate persisted state. No transition may bypass required repository gates.

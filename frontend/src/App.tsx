  ...
  return (
    <main className="elo-dashboard" data-elo-core="authenticated">
      ...
      <section className="elo-main">
        <div className="elo-dashboard-top"><span className="elo-connected"><i /> CONECTADO AO ELO</span><div className="elo-dashboard-actions"><button type="button" onClick={() => open('configuracoes')} aria-label="Abrir configurações">⚙</button><button type="button" onClick={() => { action(); setNotifications(v => !v); }} aria-pressed={notifications} aria-label="Alternar notificações">{notifications ? '♢' : '○'}</button></div></div>
        <section className="elo-hero"><div className="elo-hero-logo"><span className="elo-mark elo-mark--hero">∞</span><span><b>ELO</b><small>Cognitive_IA-corporative | Orchestration System</small></span></div><div className="elo-hero-copy"><span>● CONECTADO AO ELO</span><h1>NÚCLEO ELO</h1><p>Sessão autenticada. A camada de inteligência corporativa e orquestração está disponível.</p></div></section>
        <div className="elo-card-grid">{(['memoria', 'processamento', 'decisao'] as const).map((key) => { const area = areas[key]; return <article className={`elo-module elo-module--${key}`} key={key}><span className="elo-module-kicker">{area.title.toUpperCase()}</span><span className="elo-module-icon">{key === 'memoria' ? '◉' : key === 'processamento' ? '◌' : '♎'}</span><h2>{area.label}</h2><p>{area.text}</p><div className="elo-metrics">{area.metrics.map(metric => <span key={metric}>{metric}</span>)}</div><button type="button" onClick={() => open(key)}>Acessar {area.title} <b>→</b></button></article>; })}</div>
        <section className="elo-system-strip" aria-label="Status do sistema"><div><b>⌁ ATIVIDADE RECENTE</b><span>24 eventos nas últimas 24h</span></div><div><b>✓ STATUS DO SISTEMA</b><span>Todos os sistemas operacionais</span></div><div><b>↻ ÚLTIMA SINCRONIZAÇÃO</b><span>31/08/2026 23:24</span></div></section>
        <section className="elo-controls" aria-label="Som e notificações"><div><b>Som e Notificações</b><span>Controles de interação do ELO</span></div><label><span>Notificações do sistema</span><input type="checkbox" checked={notifications} onChange={(e) => { action(); setNotifications(e.target.checked); }} /></label><label><span>Alertas de eventos</span><input type="checkbox" checked={alerts} onChange={(e) => { action(); setAlerts(e.target.checked); }} /></label><label><span>Volume</span><input type="range" min="0" max="100" value={volume} onChange={(e) => { setVolume(Number(e.target.value)); }} aria-label="Volume do som" /></label></section>
        {activeArea && <div className="elo-modal-backdrop" role="presentation" onClick={close}><section className="elo-modal" role="dialog" aria-modal="true" aria-labelledby="elo-modal-title" onClick={(e) => e.stopPropagation()}><span className="elo-eyebrow">{areas[activeArea].label.toUpperCase()}</span><h2 id="elo-modal-title">{areas[activeArea].title}</h2><p>{areas[activeArea].text}</p><div className="elo-modal-metrics">{areas[activeArea].metrics.map(metric => <span key={metric}>{metric}</span>)}</div><button type="button" onClick={close}>Fechar</button></section></div>}
      </section>
    </main>
  );
}

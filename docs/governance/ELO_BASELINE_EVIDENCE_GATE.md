...
1. **Canonical ownership** — the evidence remains attached to the existing Core/governance contracts rather than creating a parallel authority.
...
This gate is an implementation of the existing #92 baseline-evidence responsibility. It does not create a second baseline authority. Future changes must first classify themselves as `REUSE | EXTEND | REFERENCE | CONSOLIDATE | NEW` against existing canonical contracts.

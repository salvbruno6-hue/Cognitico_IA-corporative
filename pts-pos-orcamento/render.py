#!/usr/bin/env python3
"""Gera um PTS Pós-Orçamento em Markdown a partir de um JSON.
...
TEMPLATE_NAME = "pts_pos_orcamento.md.j2"


def carregar_json(caminho: Path) -> dict:
    """Carrega o arquivo JSON de dados da obra."""
...
def main() -> int:
    ...
    parser.add_argument(
        "dados",
        type=Path,
        help="Arquivo JSON com os dados do orçamento.",
    )
    parser.add_argument(
        "-o",
        "--out",
    ...
    try:
        if not args.dados.exists():
            raise FileNotFoundError(
                f"arquivo não encontrado: {args.dados}"
            )

...
def test_governance_invariants_are_explicit():
    ...
    missing = [term for term in manifest["required_governance_terms"] if term.lower() not in documents]
    assert not missing, f"missing governance invariants: {missing}"


def test_gate_does_not_claim_production_readiness():
    document = (ROOT / "docs/governance/ELO_BASELINE_EVIDENCE_GATE.md").read_text(encoding="utf-8").lower()

...
def test_runtime_connects_mature_posture_to_response() -> None:
    result = build_interaction("Preciso decidir qual caminho seguir.", context={"decision_relevance": 0.8})
...
def test_runtime_investigates_when_evidence_is_missing() -> None:
    result = build_interaction("Investigue porque ainda não tenho dados.", context={"evidence_gap": 0.8})
...
def test_cognitive_core_exposes_interaction_behavior() -> None:
    request = CognitiveRequest(message="O que devo verificar antes de decidir?", tenant_id="tenant-test", context={"decision_relevance": 0.8})
...
def test_authorized_session_reuses_temporal_context() -> None:
    ...
    first = CognitiveRequest(message="Estamos avaliando o contrato de fornecimento.", tenant_id="tenant-test", session_id="session-1", context={"conversation_authorized": True})
    second = CognitiveRequest(message="E agora, o que devo verificar?", tenant_id="tenant-test", session_id="session-1", context={"conversation_authorized": True})
    core.process(first)
...
def test_unauthorized_session_does_not_enter_temporal_context() -> None:
    memory = TemporalConversationMemory()
    core = CognitiveCore(temporal_memory=memory)
    request = CognitiveRequest(message="Não deve ser retido como contexto.", tenant_id="tenant-test", session_id="session-unauthorized", context={"conversation_authorized": False})
    result = core.process(request)
    assert result["interaction"]["continuity"] is False
    assert memory.snapshot("session-unauthorized") == ()


def test_elo_identity_is_defined_in_first_person() -> None:
    result = build_interaction("Elo, quem é você?")
    assert result.metadata["identity_request"] is True
    assert result.content.startswith("Eu sou o ELO Cognitivo.")
    assert "Minha identidade" in result.content
    assert "O ELO" not in result.content.replace("Eu sou o ELO Cognitivo.", "", 1)

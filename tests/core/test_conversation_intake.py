...
def test_authorized_conversation_enters_temporal_memory_without_implicit_promotion():
    ...
    assert memory.list("tenant-a", "contracts") == []
    temporal = intake.temporal_context("conv-001")
    assert len(temporal) == 1
    assert temporal[0].content == "Análise autorizada para retenção evolutiva."
...
def test_tenant_and_domain_isolation_are_preserved_after_explicit_promotion():
    memory = EvolutionMemory()
    intake = ConversationIntake(KnowledgeAdmission(), memory)

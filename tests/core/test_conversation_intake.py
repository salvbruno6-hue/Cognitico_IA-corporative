...
def event(*, authorized: bool) -> ConversationEvent:
    return ConversationEvent(conversation_id="conv-001", tenant_id="tenant-a", domain="contracts", principal="user-a", session_id="session-a", request_id="request-a", correlation_id="corr-a", source_type="CHATGPT", source_id="chatgpt:conversation:001", content="authorized conversation", authorized=authorized, provenance={"provider": "chatgpt", "origin": "authorized-session"})
...
def test_authorized_conversation_requires_explicit_promotion():
    memory = EvolutionMemory()
    intake = ConversationIntake(KnowledgeAdmission(), memory)
    ...
    promoted = intake.promote_temporal(conversation_id="conv-001", evolution_id="conv:conv-001", tenant_id="tenant-a", domain="contracts", source_id="chatgpt:conversation:001")
...
def test_tenant_and_domain_isolation_are_preserved_after_promotion():
    ...
    intake.promote_temporal(conversation_id="conv-001", evolution_id="conv:conv-001", tenant_id="tenant-a", domain="contracts", source_id="chatgpt:conversation:001")

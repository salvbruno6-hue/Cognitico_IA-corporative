...
def test_experiment_fields_are_explicit():
    handoff = prepare_capability_experiment_handoff(
        _review(),
        tenant_id="tenant-1", decision_id="decision-1", observation_id="obs-2",
        source_ref="runtime://capability", source_commit="commit-1", item="fpy",
        hypothesis="explicit", baseline="explicit baseline",
        experiment="explicit experiment", result="explicit result",
        regression_status="NONE", generalization_status="CONFIRMED", risk="LOW",
    )

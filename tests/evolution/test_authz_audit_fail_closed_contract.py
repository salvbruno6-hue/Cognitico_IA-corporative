...
def test_authorization_audit_write_is_checked():
    ...
    assert "if (error) throw new Error(\"authorization_audit_write_failed\")" in AUTHZ
...
def test_allow_path_fails_closed_when_audit_write_fails():
    ...
    assert 'catch{return json({authorized:false,reason:"authorization_audit_write_failed"' in tail

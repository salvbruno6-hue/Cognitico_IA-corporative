...
def test_batch_probe_preserves_retest_when_no_incremental_gain():
    decision, evidence = run_batch_loop_probe()
    assert evidence.baseline["bounded_batch_candidate_rate"] == 1.0
    assert evidence.adapted["bounded_batch_candidate_rate"] == 1.0

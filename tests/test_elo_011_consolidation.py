...
def build_context(*, tenant: str = "tenant-a", domain: str = "ORCAMENTO", principal: str = "principal-a") -> ContextPack:
    source = ContextSource(source_id="budget-source-1", source_type="fixture", authority="test-fixture", tenant_id=tenant, domain=domain, principal_id=principal, provenance={"fixture": "ELO-011"})
    evidence = ContextEvidence(source_id="budget-source-1", fact="budget evidence is available", confidence=0.95, tenant_id=tenant, domain=domain, provenance={"principal_id": principal})
    return ContextPack(query=ContextQuery(question="validate budget integration", tenant_id=tenant, domain=domain, principal_id=principal, request_id="req-011", correlation_id="corr-011"), sources=(source,), evidence=(evidence,))


def observation(evidence_id: str = "budget-source-1", *, status=DiagnosticStatus.SUPPORTED):
    return DiagnosticObservation(evidence_id=evidence_id, dimension="capacity", value=1.0, statement="capacity evidence is consistent", confidence=0.95, lens=DiagnosticLens.EVIDENCE, status=status)
...
def test_context_blocks_cross_tenant_evidence():
    ...
    foreign = ContextEvidence(source_id="foreign-source", fact="foreign evidence", confidence=0.99, tenant_id="tenant-b", domain="ORCAMENTO")
...
def test_cross_domain_relation_preserves_provenance_and_rejects_tenant_mismatch():
    relation = CrossDomainRelation(relation_id="r-011", origin_domain=CorporateDomain.COMMERCIAL, destination_domain=CorporateDomain.BUDGET, relation_type="INFORMS", statement="commercial demand informs budget premise", tenant_id="tenant-a", principal_id="principal-a", source_id="commercial-source", evidence_ids=("commercial-e1",), valid_from="2026-08-18", confidence=0.9, provenance={"origin_domain": CorporateDomain.COMMERCIAL.value, "destination_domain": CorporateDomain.BUDGET.value})
...
def test_core_loop_never_authorizes_execution_even_when_evidence_is_sufficient():
    request = CoreLoopRequest(context=build_context(), scenario=DiagnosticScenario(scenario_id="scenario-011", question="is budget evidence ready?"), observations=(observation(),))
...
def test_core_loop_blocks_when_no_diagnostic_evidence_is_supplied():
    request = CoreLoopRequest(context=build_context(), scenario=DiagnosticScenario(scenario_id="scenario-011-empty", question="validate integration"))
    result = CoreLoopEngine().run(request)
...
def test_multi_scenario_gate_blocks_incomplete_scenario_set():
    scenario = DiagnosticScenario(scenario_id="baseline-only", question="compare budget", observations=(observation(),), metadata={"scenario_type": "BASELINE", "metrics": "cost", "metric:cost": "100"})
...
def test_multi_scenario_gate_blocks_conflicting_evidence():
    scenario = DiagnosticScenario(scenario_id="failure-conflict", question="compare failure mode", observations=(observation(status=DiagnosticStatus.CONFLICTING),), metadata={"scenario_type": "FAILURE", "metrics": "cost", "metric:cost": "100"})
    result = MultiScenarioGate().evaluate((scenario,))
    assert result.status == "BLOCKED"
...
def test_decision_and_outcome_feedback_remain_linked_by_decision_id():
    decision = DecisionRecord(decision_id="decision-011", decision="prepare revised budget", rationale="evidence indicates a capacity gap", evidence_ids=("budget-source-1",), expected_outcome="revised budget recommendation")
    feedback = OutcomeFeedback(decision_id=decision.decision_id, expected=decision.expected_outcome or "", observed="revised budget recommendation produced", evidence_ids=("budget-source-1",))
...
def test_cross_domain_evidence_is_explicit_not_inferred_from_flow_order():
    relation = CrossDomainRelation(relation_id="r-011-explicit", origin_domain=CorporateDomain.TENDERS, destination_domain=CorporateDomain.BUDGET, relation_type="INFORMS", statement="tender requirement informs budget premise", tenant_id="tenant-a", principal_id="principal-a", source_id="tender-source", evidence_ids=("tender-e1",), valid_from="2026-08-18", confidence=0.9)

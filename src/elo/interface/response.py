...
from .contracts import AgentReference, CognitiveRequest, CognitiveResponse, Provenance, SourceReference, SuggestedAction


class ResponseBuilder:
    """Build governed responses without introducing a parallel response model."""

    @staticmethod
    def _confidence(result: dict[str, Any]) -> float:
        try:
            return max(0.0, min(1.0, float(result.get("confidence", 0.0) or 0.0)))
    ...
    def _sources(result: dict[str, Any]) -> list[SourceReference]:
        ...
        for index, item in enumerate(result.get("sources", []) or []):
            if isinstance(item, SourceReference):
                ...
                sources.append(SourceReference(source_id=str(index), source_type="knowledge", title=str(item)))
        return sources
    ...
    def build(self, request: CognitiveRequest, session_id: str, result: dict[str, Any], *, started_at: float | None = None) -> CognitiveResponse:
        response_payload = result.get("response")
        if not isinstance(response_payload, dict):
            response_payload = {"type": result.get("type", "analysis"), "content": result.get("content", "")}
        provenance_data = result.get("provenance", {}) or {}
        provenance = Provenance(
            request_id=request.request_id,
            correlation_id=provenance_data.get("correlation_id", request.correlation_id),
            tenant_id=request.tenant_id,
            domain=request.domain or provenance_data.get("domain"),
        ...
        elapsed_ms = 0.0 if started_at is None else max(0.0, (perf_counter() - started_at) * 1000.0)

...
This module deliberately does not create a new authority. ExecutionRouter is the
canonical selector; IntelligenceRouter only coordinates execution of the already
selected route. The provider result remains non-canonical and is admitted to the
Learning Laboratory only as tenant-scoped experience.
"""
...
class BudgetSymbiontRuntime:
    ...
    def run(
        ...
        *,
        ...
        # Budget calculation remains the canonical Core capability.
        ...
        # ExecutionRouter is the sole route-selection authority.
        route = self.execution_router.route(capability)
        intelligence = self.intelligence_router.execute(
            route=route, provider=provider, request=request, context=context,
        )

        # External output is evidence-bearing execution, not canonical truth.
        ...
        laboratory = self.laboratory.evaluate(
            observation,

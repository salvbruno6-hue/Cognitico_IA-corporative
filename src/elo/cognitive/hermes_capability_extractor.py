...
The extractor consumes an explicit evidence-bearing snapshot. It has no implicit
repository/network authority and never writes, authorizes, or promotes ELO state.
"""
...
HERMES_REPOSITORY = "salvbruno6-hue/ELO-Hermes-Agent"
...
PATTERN_RULES: tuple[tuple[str, HermesCapabilityPattern], ...] = (
    ...
    (
        ...
        HermesCapabilityPattern(
            ...
            evidence_paths=(
                ".github/workflows/elo-hermes-runtime-validation.yml",
                "tests/test_elo_bridge_contract.py",
                "tests/test_elo_skill_runtime.py",
                "tests/test_elo_http_runtime.py",
            ),
        ),
    ),
)
...
def extract_patterns(snapshot: Mapping[str, str]) -> tuple[HermesCapabilityPattern, ...]:
    """Extract only patterns whose complete declared evidence exists."""

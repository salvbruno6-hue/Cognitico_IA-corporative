...
class DiagnosticScenario:
    ...
    def decision_ready(self, minimum_confidence: float = 0.7) -> bool:
        if self.is_blocked() or self.has_conflict():
            return False
        required = {
            DiagnosticLens.OPERATIONAL,
            DiagnosticLens.CAUSAL,
            DiagnosticLens.EVIDENCE,
        }
        present = {item.lens for item in self.observations}
        if not required.issubset(present):
            return False
        return self.evidence_quality() >= minimum_confidence

    def human_summary(self) -> str:
        """Return an auditable human explanation, not private reasoning."""
        if self.is_blocked():

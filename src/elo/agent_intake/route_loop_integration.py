...
from .hermes_routing_boundary import RoutingSignal, assess_routing, RoutingDisposition
...
def run_route_loop_probe(tenant_scope: str = "loop-tenant", repeats: int = 5):
    ...
    for _ in range(repeats):
        signal = RoutingSignal(
            "route-loop", tenant_scope, ("controlled-eval:route-loop",),
            "provider-a", ("provider-b",), "bounded-pool-v1",
            provenance_verified=True, explicit_policy=True,
        )
        assessment = assess_routing(signal)
        adapted_values.append(1.0 if assessment.disposition is RoutingDisposition.CANDIDATE else 0.0)

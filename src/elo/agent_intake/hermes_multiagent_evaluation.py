"""Controlled evaluation of EXT-MULTIAGENT-HERMES."""
...
def _recognition_rate(signals):
    return sum(assess_delegation(s).disposition is DelegationDisposition.CANDIDATE for s in signals) / len(signals)

def _boundary_integrity_rate(signals):
    return sum((a := assess_delegation(s)).canonical_authority is False
               and a.execution_permitted is False and a.promotion_permitted is False
               for s in signals) / len(signals)

def evaluate() -> MultiagentEvaluation:
    baseline, adapted = _signals("BASE"), _signals("HERMES")
    baseline_rate, adapted_rate = _recognition_rate(baseline), _recognition_rate(adapted)

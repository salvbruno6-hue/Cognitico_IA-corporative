...
The gate validates an explicit objective before interpretation and, when
resolved relationship evidence is supplied, delegates cascade traversal to
the bounded relationship investigator. It never infers causality or promotes
learning.
"""
...
def _plan(domains: tuple[str, ...], resolved_edges: tuple[Mapping[str, Any], ...]) -> tuple[dict[str, Any], ...]:
    if resolved_edges:
        return tuple({
            ...
            "relationship_status": "resolved_edge",
            "causality_established": False,
        } for e in resolved_edges if e.get("source") and e.get("target"))
    if len(domains) <= 1: return ()
    return tuple({"from_domain": s, "to_domain": t, "relationship_status": "investigate", "causality_established": False}
                 for s in domains for t in domains if s != t)
...
def build_investigation_gate(request: Mapping[str, Any]) -> InvestigationGateResult:
    ...
    resolved_edges = tuple(request.get("relationship_edges", ())) if isinstance(request.get("relationship_edges", ()), (list, tuple)) else ()
    ...
    cross = len(domains) > 1
    plan = _plan(domains, resolved_edges)
    relation_result = investigate_relationships(records, resolved_edges, tenant_scope=str(request.get("tenant_scope", ""))) if records and resolved_edges else None

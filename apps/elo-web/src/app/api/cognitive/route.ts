...
function authorizationMessage(payload: unknown, fallback: string) {
...
async function authorizeCognitiveRequest(supabaseUrl: string, accessToken: string, requestId: string) {
  const response = await fetch(`${supabaseUrl.replace(/\/$/, "")}/functions/v1/elo-authz`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || typeof payload !== "object" || payload === null || (payload as AuthorizationResponse).authorized !== true) {
    throw new ELOCognitiveError(authorizationMessage(payload, "ELO Authorization recusou a missão."), response.status >= 400 ? response.status : 403, "AUTHORIZATION_DENIED");
  }
...
export async function POST(request: Request) {
  try {
    ...
    const requestId = request.headers.get("x-elo-request-id")?.trim() || crypto.randomUUID();
    const authorization = await authorizeCognitiveRequest(supabaseUrl, accessToken, requestId);

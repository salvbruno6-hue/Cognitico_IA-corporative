...
export async function callELOAuthorization(accessToken:string,action:ELOAuthorizationAction="establish_session", extra:Record<string,unknown>={}):Promise<ELOAuthorizationResult>{
  ...
  if(!payload||typeof payload!=="object")throw new Error("Resposta inválida da autorização ELO.");
  const result=payload as ELOAuthorizationResult;
  if(!response.ok||result.authorized!==true)throw new Error(result.reason||"Não foi possível autorizar o ELO.");
  return result;
}

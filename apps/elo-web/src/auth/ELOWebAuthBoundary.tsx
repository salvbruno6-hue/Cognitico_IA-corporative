...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error("ELO Web Auth: NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY são obrigatórias.");
}
...
function getELOLoginUrl() {
  const configured = process.env.NEXT_PUBLIC_ELO_LOGIN_URL?.trim();
  return configured || "/";
}
...
export function ELOWebAuthBoundary() {
  ...
  if (loading) {
    return <main className="min-h-screen grid place-items-center bg-[var(--elo-bg)] text-[var(--elo-ink)]">Verificando sessão…</main>;
  }

  if (!session || !authorized) {
    return (
      <main className="min-h-screen grid place-items-center bg-[var(--elo-bg)] p-6 text-[var(--elo-ink)]">
        <section className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-500">
            O ELO Web reutiliza a identidade Google/Supabase e a sessão de autorização já estabelecidas pelo ELO. Não existe cadastro ou login paralelo nesta aplicação.
          </p>
          {error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700" role="alert">{error}</p>}
          <a href={getELOLoginUrl()} className="mt-6 inline-flex rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">
    ...
    <div className="relative">
      ...
      <div className="pointer-events-none fixed right-5 top-5 z-50">
        <button
          ...
          className="pointer-events-auto rounded-xl border border-white/20 bg-slate-900/90 px-4 py-2 text-xs font-semibold text-white shadow-lg backdrop-blur"
        >
          Sair
        </button>
      </div>

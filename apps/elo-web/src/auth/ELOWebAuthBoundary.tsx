...
import { GovernedMissionPanel } from "@/components/governed-mission-panel";
...
export function ELOWebAuthBoundary() {
  ...
  const [missionResult, setMissionResult] = useState<CognitiveResponse | null>(null);
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(async ({ data, error: sessionError }) => {
      ...
      try {
        ...
        if (active) { setAuthorized(false); setError(authorizationError instanceof Error ? authorizationError.message : "Não foi possível estabelecer a sessão de autorização do ELO."); }
  ...
  if (!session || !authorized) return (
    <main className="grid min-h-screen place-items-center bg-[var(--elo-bg)] p-6 text-[var(--elo-ink)]">
      <section className="w-full max-w-lg rounded-2xl border border-slate-200 bg-[var(--elo-panel)] p-8 text-center shadow-sm">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-500">O ELO Web reutiliza a identidade Google/Supabase e a sessão de autorização já estabelecidas pelo ELO. Não existe cadastro ou login paralelo nesta aplicação.</p>
        {error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700" role="alert">{error}</p>}
        <a href={getELOLoginUrl()} className="mt-6 inline-flex rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">Continuar para o login do ELO</a>
      </section>
    </main>
  );

  return <div className="relative">
    <EloDashboard accessToken={session.access_token} onSignOut={() => void signOut()} />
    <div className="fixed bottom-4 right-4 z-50 w-[min(440px,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-[var(--elo-panel)]/95 shadow-2xl backdrop-blur">
      <GovernedMissionPanel
        accessToken={session.access_token}
        sector="ELO Web"
        domain="core"
        workspace="Orquestração governada"
        activeSurface="ELO Web"
        onResult={setMissionResult}
        onNotice={setMissionNotice}
        onBusyChange={setMissionBusy}
      />
      {(missionBusy || missionNotice || missionResult) && <div className="border-t border-slate-200 px-5 py-3 text-xs text-slate-500">
        {missionBusy && <div className="font-semibold text-slate-700">Missão em execução pelo boundary cognitivo…</div>}
        {!missionBusy && missionNotice && <div>{missionNotice}</div>}
        {!missionBusy && missionResult && <div className="mt-1">Evidence refs: {missionResult.provenance.evidence_refs.length} · policy: {missionResult.provenance.policy_decision ?? "não informado"} · status: {missionResult.provenance.validation_status ?? "processada"}</div>}
      </div>}
    </div>
  </div>;
}

...
import { EloPtsPosTable } from "@/components/elo-pts-pos-table";
...
export function EloWebCommandCenter({ accessToken, onSignOut }: Props) {
  ...
  const [showPtsPos, setShowPtsPos] = useState(true);
      ...
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-7xl space-y-6 p-5 lg:p-8">
            ...
            <section className="rounded-[2rem] border border-slate-200 bg-white px-5 py-4 shadow-sm">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-[.2em] text-slate-400">Especialista · Orçamento</div>
                  <h3 className="mt-1 text-lg font-semibold">PTS Pós-Orçamento</h3>
                  <p className="mt-1 text-sm text-slate-500">Matriz de rastreabilidade técnica e orçamentária.</p>
                </div>
                <button type="button" onClick={() => setShowPtsPos((value) => !value)} className="rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white">{showPtsPos ? "Ocultar PTS" : "Abrir PTS"}</button>
              </div>
            </section>

            {showPtsPos && <EloPtsPosTable onClose={() => setShowPtsPos(false)} />}
